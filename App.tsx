...
const App: React.FC = () => {
  ...
  const remindersShownRef = useRef<{rest?: boolean, duty?: boolean, lateNight?: boolean}>({});

  const getIdleState = useCallback(() => {
    const hour = new Date().getHours();
    if (user && user.role === UserRole.ADMIN && (hour >= 23 || hour < 4 || isProtocolXManuallyActive)) {
  ...
  const handleFunctionCall = (fc: any) => {
    ...
    try {
      ...
      speakTextTTS(user!, confirmationText, config,
      ...
      const errorMsg: ChatMessage = { role: 'model', text: "Sorry, I encountered an error trying to perform that action.", timestamp: Date.now() };
       speakTextTTS(user!, errorMsg.text, config,
  ...
  const processInput = useCallback(async (text: string) => {
    ...
    try {
        ...
        const isAngry = finalState === HUDState.WARNING;
  ...
  }, [user, isProcessing, getIdleState, isProtocolXManuallyActive, handleLogout, abuseWarningCount, config]);

  // Proactive Reminders for Admin (Updated v9.0)
  useEffect(() => {
    const reminderInterval = setInterval(() => {
        ...
        // 2 AM Late Night Concern (New)
        if (hour === 2 && !remindersShownRef.current.lateNight) {
            const msg = "Sir... it is 2 AM. You are still awake? Please, you need sleep. Main concern kar rahi hoon.";
            speakTextTTS(user, msg, config, () => {}, () => {}, false);
            remindersShownRef.current.lateNight = true;
        }
    ...
    if (currentUser.role === UserRole.ADMIN) {
        ...
        if (notifications && notifications.length > 0) {
            ...
            await new Promise<void>(resolve => {
                speakTextTTS(currentUser, briefingText, config, () => { setMessages(prev => [...prev, briefingMsg]); setHudState(HUDState.SPEAKING); }, () => { resolve(); }, false);
            });
        }
    }
    try {
  ...
  const handleProtocolXToggle = (isActive: boolean) => { setIsProtocolXManuallyActive(isActive); if (isActive) { playSystemNotificationSound(); if(user) speakTextTTS(user, "Protocol X Activated, Sir.", config, () => {}, () => {}, false); setHudState(HUDState.LATE_NIGHT); } else { setHudState(getIdleState()); } };
