...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-6 z-[60] overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-20"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] border border-nexa-cyan/20 rounded-full animate-spin-slow"></div><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] border border-dashed border-nexa-cyan/20 rounded-full animate-spin-reverse-slow"></div></div>
      ...
      <div className="absolute top-8 text-center animate-fade-in z-50"><div className="text-[10px] text-nexa-cyan/50 font-mono tracking-[0.4em]">CREATED BY</div><div className="text-xl font-bold text-white tracking-[0.2em]">CHANDAN LOHAVE</div></div>
      <div className="relative w-full max-w-sm z-50">
        ...
        <div className={`backdrop-blur-md border p-6 relative transition-all duration-500 ${mode === 'ADMIN' ? 'bg-red-900/10 border-red-500/20' : 'bg-black/60 border-nexa-cyan/10'}`}>
          ...
          {mode === 'INIT' && (<div className="flex flex-col items-center py-10 animate-fade-in"><div onClick={initiateSystem} className="relative w-32 h-32 flex items-center justify-center cursor-pointer group"><div className="absolute inset-0 bg-nexa-cyan/10 rounded-full blur-xl group-hover:bg-nexa-cyan/30 transition-all duration-500"></div><div className="absolute w-full h-full border-2 border-nexa-cyan rounded-full border-t-transparent animate-spin"></div><div className="absolute w-[80%] h-[80%] border-2 border-dashed border-nexa-cyan/50 rounded-full animate-spin-reverse-slow"></div><div className="absolute w-[40%] h-[40%] bg-nexa-cyan rounded-full animate-pulse shadow-[0_0_20px_currentColor]"></div></div><div className="mt-8 text-center space-y-2"><h1 className="text-4xl font-bold text-white tracking-widest">NEXA</h1><div className="text-nexa-cyan/60 text-xs font-mono tracking-[0.3em] group-hover:text-nexa-cyan transition-colors">{loading ? 'INITIALIZING...' : initStatusText}</div></div></div>)}
