...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleAdminLogin = () => {
    if (formData.username === 'Chandan' && formData.password === 'Nexa') {
      completeLogin({ name: 'Chandan', mobile: '0000000000', role: UserRole.ADMIN });
    } else {
      playErrorSound();
  ...
  const handleUserLogin = () => {
    ...
    if (!mobileRegex.test(formData.mobile)) {
      playErrorSound();
      setError('// ERROR: PLEASE ENTER A VALID 10-DIGIT MOBILE NUMBER');
      return;
    }
    completeLogin({ name: formData.fullName, mobile: formData.mobile, role: UserRole.USER });
  };
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-6 z-[60] overflow-hidden">
        ...
        <div className={`backdrop-blur-md border p-6 relative transition-all duration-500 ${mode === 'ADMIN' ? 'bg-red-900/10 border-red-500/20' : 'bg-black/60 border-nexa-cyan/10'}`}>
          ...
          {mode === 'INIT' && (
            <div className="flex flex-col items-center py-10 animate-fade-in">
               ...
               <div className="mt-8 text-center space-y-2">
                 ...
                 <div className="text-nexa-cyan/60 text-xs font-mono tracking-[0.3em] group-hover:text-nexa-cyan transition-colors">{loading ? 'INITIALIZING...' : initStatusText}</div>
               </div>
            </div>
          )}
          ...
          {mode === 'ADMIN' && (
            <div className="animate-slide-up relative z-10">
               ...
               <div className="text-center mb-6"><div className="inline-flex items-center gap-2 border border-red-500/50 px-3 py-1 bg-red-500/10 backdrop-blur-sm"><div className="w-2 h-2 bg-red-500 animate-pulse rounded-full"></div><span className="text-red-500 text-[10px] font-mono tracking-[0.2em] uppercase">Security Level 8</span></div></div>
               <div className="absolute top-10 right-4 opacity-5 pointer-events-none"><svg className="w-24 h-24 text-red-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17a2 2 0 100-4 2 2 0 000 4zm6-9a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V10a2 2 0 012-2h1V6a5 5 0 0110 0v2h1zM8 6a4 4 0 018 0v2H8V6z"/></svg></div>
               ...
               <div className="pt-8 space-y-4 relative z-20">
                 <button onClick={handleAdminLogin} disabled={loading} className="w-full py-4 bg-red-600 text-white font-bold tracking-[0.2em] hover:bg-red-500 hover:shadow-[0_0_25px_rgba(220,38,38,0.8)] transition-all clip-corner relative overflow-hidden group" style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
                   <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                   <span className="relative z-10 flex items-center justify-center gap-2">{loading ? 'AUTHENTICATING...' : <>AUTHORIZE OVERRIDE <span className="text-xs opacity-70">{'>>'}</span></>}</span>
                 </button>
                 <div className="mt-2"><button onClick={() => setMode('USER')} className="text-[9px] text-red-500/60 hover:text-red-500 font-mono tracking-widest uppercase transition-colors flex items-center justify-center gap-2 w-full group"><span className="group-hover:-translate-x-1 transition-transform">&lt;&lt;</span> ABORT SEQUENCE</button></div>
               </div>
            </div>
          )}
        </div>
      ...
      <div className="absolute bottom-16 text-center text-[8px] font-mono text-nexa-cyan/30 tracking-widest animate-fade-in z-50">© 2025 CHANDAN LOHAVE. ALL RIGHTS RESERVED.</div>
      <InstallPWAButton />
